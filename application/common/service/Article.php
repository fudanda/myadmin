...
class Article extends ArticleModel
{
    ...
    {
        $data = [
            'article_id' =>  input('param.article_id/s', null),
        ];
        $condition[] = ['article_id', 'like', '%' . $data['article_id'] . '%'];
        /** 查询列表数据 */
    ...
    {
        $data = [
            'article_title' =>  input('post.article_title/s', '', 'trim'),
        ];
    ...
    {
        /* 验证用户ID存在性 */
        if (!$id = input('param.article_id/s')) return ['code' => 1, 'msg' => 'ID不能为空'];
        $result = self::get($id);
    ...
    public function updateInfo()
    {
        ...
        if (!$id = input('param.article_id/s')) return ['code' => 1, 'msg' => 'ID不能为空'];
        $data = [
            'article_data'   =>  input('post.article_data/s', '', 'trim'),
        ];
        /* 执行更新 */
        $result = self::where('article_id', $id)
            ->update($data);
        $result = $this->getLastSql();
        /* 确认并返回结果 */
